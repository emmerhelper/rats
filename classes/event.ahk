class event extends object {
      ...
      __New(gridview) {
            ...
            this.isAfternoon := calculateAfternoon()
            this.day := calculateDay()
            this.time := calculateTime()
            this.room := calculateRoom()
            this.person := calculatePerson()
            this.capacity := calculateCapacities()
            this.registration := calculateRegistration()
            ...
            calculateDay(){
                  day := {}
                  day.number := calculateDayNumber()
                  day.name := calculateDayName(day.number)
                  return day
            }
      ...
      scheduleRegular(session,gridview){
            ...
            ;; Room
            findTextElement(userArea,"GS_SCHEDELEM-ROOM_SHORT").text := this.room
            session.findById("wnd[0]").sendVKey(0)
            session.findByID("wnd[1]/usr/btnBUTTON_1").Press()
